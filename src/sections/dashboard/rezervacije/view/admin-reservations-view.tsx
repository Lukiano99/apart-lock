 ...
 * {!isPending && data && (
        <Grid xs={12}>
          <BookingDetails
            ...
            tableData={data.reservations.map((res) => ({
              checkIn: res.check_in.toLocaleDateString("en-CA"), // Očekuje se IDateValue
              checkOut: res.check_out.toLocaleDateString("en-CA"), // Očekuje se IDateValue
              customer: {
                name: `${res.customer.firstName} ${res.customer.lastName}`, // Očekuje se string
                phoneNumber: res.customer.phone, // Očekuje se string
                avatarUrl: "", // Očekuje se string
              },
              destination: {
                name: res.Room.apartment.name, // Očekuje se string
                coverUrl:
                  "https://images.unsplash.com/photo-1560448204-e02f11c3d0e2", // Očekuje se string
              },
              status: res.status, // Očekuje se string
              id: res.id, // Očekuje se string
            }))}
            headLabel={[
              { id: "destination", label: "Apartman" },
